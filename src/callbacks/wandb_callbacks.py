...
class LogMetricsAndRunningTime(Callback):
    ...
    def on_test_end(self, trainer: "pl.Trainer", pl_module: "pl.LightningModule") -> None:
        ...
        for pred, target in zip(self.preds, self.targets):
            ...
            for n in self.NDCGs:
                # index_tensor = torch.tensor(range(pred.size(0))).unsqueeze(1).repeat(1, pred.size(1))
                # n.update(pred, target, index_tensor)
                if flag == 0:
                    np.set_printoptions(threshold=1e6)
                    with open("ndcg.txt", "w") as file:
                        print("pred_size:", pred.size())
                        print("traget_size:", target.size())
                        file.write(f"preds:\n")
                        file.write(str(pred.cpu().numpy()))
                        file.write(f"target:\n")
                        file.write(str(target.cpu().numpy()))
                        index_tensor = torch.tensor(range(pred.size(0))).unsqueeze(1).repeat(1, pred.size(1))
                        file.write(f"index: {index_tensor}\n")
                        # 计算并输出NDCG
                        n.update(pred, target, index_tensor)
                        result = n.compute()
                        file.write(f"NDCG: {result}\n")
                        flag += 1
            for f in self.F1s:
