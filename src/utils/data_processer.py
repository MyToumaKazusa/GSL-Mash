...
#分词、去除标点符号、去除停用词、替换缩写词和词形还原等步骤。返回的结果是一个包含处理后单词的列表。
...
class DataReader:
    ...
    #根据选择的模型对description进行embedding
    ...
    #提取api的description，并进行embedding，返回embedding和拼接平均处理后的平均embedding
    ...
    #根据api的类别进行分类，并返回一个分类后的名为domain的embedding
